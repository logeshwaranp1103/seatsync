-- ====================================================================
-- SEATSYNC UNIFIED MIGRATION 44: LIVE OCCUPANCY REAL DATABASE ENGINE REPAIR
-- ====================================================================
...
-- 1. REPAIR: get_live_occupancy_snapshot
...
DECLARE
    ...
    v_slot_name TEXT := 'All Operational Slots';
...
BEGIN
    ...
    -- Determine active Slot ID & Slot Occurrence if provided or auto-detecting
    ...
    ELSE
        -- Check if there is an active slot for current time
        ...
        IF v_slot_id IS NOT NULL THEN
            SELECT ARRAY_AGG(id) INTO v_matched_slot_ids
            FROM public.slots
            WHERE name = v_slot_name;
        ELSE
            v_slot_name := 'All Daily Slots';
        END IF;
    END IF;

    -- Aggregate Overall Capacity Metrics for selected library / floor / room
    SELECT 
    ...
    WHERE b.booking_date = v_date
      AND (
        v_matched_slot_ids IS NULL OR
        b.slot_id = ANY(v_matched_slot_ids) OR
        b.slot_occurrence_id IN (SELECT id FROM public.slot_occurrences WHERE slot_id = ANY(v_matched_slot_ids))
      )
    ...
    FROM (
        ...
        LEFT JOIN public.seat_maintenance sm ON sm.seat_id = s.id AND (sm.status IS DISTINCT FROM 'Resolved' AND sm.completed_at IS NULL)
        LEFT JOIN public.bookings b ON b.seat_id = s.id AND b.booking_date = v_date AND (v_matched_slot_ids IS NULL OR b.slot_id = ANY(v_matched_slot_ids))
        WHERE (v_library_id IS NULL OR f.library_id = v_library_id)
          AND (p_floor_id IS NULL OR f.id = p_floor_id)
          AND (p_room_id IS NULL OR r.id = p_room_id)
...
-- 2. REPAIR: get_live_seat_statuses
...
BEGIN
    ...
    FROM (
        ...
        LEFT JOIN public.bookings b ON b.seat_id = s.id 
          ...
          AND (v_matched_slot_ids IS NULL OR b.slot_id = ANY(v_matched_slot_ids))
...
-- 3. REPAIR: get_current_occupants
